...
class OnnxControllerRos2(OnnxController, Node):
    ...
    def __init__(
        ...
        # n_rays = len(_rays_theta)
        # self.rays_phi = ti.ndarray(dtype=ti.f32, shape=n_rays)
        # self.rays_theta = ti.ndarray(dtype=ti.f32, shape=n_rays)
        # self.rays_phi.from_numpy(_rays_phi)
        # self.rays_theta.from_numpy(_rays_theta)
